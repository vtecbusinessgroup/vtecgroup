...
<a href="https://vtecgroup.co.ke/contact" style="display:inline-block;background:${TEAL};color:${NAVY};text-decoration:none;font-weight:800;padding:14px 24px;border-radius:10px;">${esc(report.vtecRecommendation.ctaText)} &rarr;</a>
</div>
</td></tr>
...
<tr><td style="color:#6b7280;vertical-align:top;">12 Month Goal</td><td>${esc(a.goal || "(not provided)")}</td></tr>
      ...
      POST: async ({ request }) => {
        try {
          ...
          const required: (keyof Answers)[] = [
            "stage", "industry", "challenge", "revenue", "team", "name", "email",
          ];
          ...
          const aiRes = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${key}`,
            {
              ...
              body: JSON.stringify({
                ...
                contents: [{ role: "user", parts: [{ text: buildUserPrompt(answers) }] }],
