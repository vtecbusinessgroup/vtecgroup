...
export const generateDiagnostic = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { stage, industry, challenge, revenue, team, goal, name, email, whatsapp } = data;

    if (!name || !email) {
      return { error: "Missing name or email" } as const;
    }
...
Return ONLY a JSON object with exactly these fields:
{
  ...
  "vtecRecommendation": {
    ...
    if (!aiRes.ok) {
      ...
      return { error: "AI service error" } as const;
    }
    ...
    if (!text) {
      ...
      return { error: "Invalid AI response" } as const;
    }
    ...
    try {
      ...
      return { error: "Invalid AI response format" } as const;
    }

    if (resendKey) {
      try {
        await sendEmails(
          { report, name, email, whatsapp, stage, industry, challenge, revenue, team, goal },
          resendKey,
        );
      } catch (err) {
...
function buildUserEmail(name: string, r: DiagnosticReport): string {
  const steps = Array.isArray(r?.actionSteps)
    ? r.actionSteps.map((s) => `<li style="margin-bottom:8px;">${esc(s)}</li>`).join("")
    : "";
  const rm = r?.roadmap ?? ({} as DiagnosticReport["roadmap"]);
